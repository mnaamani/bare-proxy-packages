...
url in the request line (`GET http://origin.example/v1/info HTTP/1.1`, what RFC 9112 section 3.2.2
...
TLS to run over - for `https:` use [`barex-https-proxy-agent`](../barex-https-proxy-agent),
whose CONNECT tunnel leaves TLS to the target intact.

```
npm i barex-http-proxy-agent
```
...
`barex-any-proxy-agent` to route HTTP requests by forwarding and HTTPS requests by CONNECT.
...
Re-exported from [`barex-proxy-agent`](../barex-proxy-agent). Note that a forwarding proxy
