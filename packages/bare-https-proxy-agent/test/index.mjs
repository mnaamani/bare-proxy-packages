...
test('a proxy that will not open the tunnel says what it answered', async (t) => {
  ...
  const err = await failure(t, proxy.port)
  t.ok(/refused a tunnel to origin.example:80: HTTP 403/.test(err.message), err.message)
})
...
test('a header value that would end the line early is refused', async (t) => {
  ...
  const err = await fetch('http://origin.example/', { agent: agents.http }).then(
    () => null,
...
async function failure(t, port, url = 'http://origin.example/') {
