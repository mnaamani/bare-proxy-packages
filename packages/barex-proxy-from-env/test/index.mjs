...
test('no_proxy of * sends everything direct, and an unset one nothing', (t) => {
  ...
  t.is(parseNoProxy('  ,  '), null)
  t.absent(bypassed(null, 'origin.example'), 'a null bypass exempts nothing')
})
...
test('getProxyForUrl answers with a url, or with the empty string', (t) => {
  ...
  t.is(getProxyForUrl('https://origin.example/v1/info'), 'http://secure.lan:3128')
  t.is(getProxyForUrl(new URL('https://origin.example')), 'http://secure.lan:3128', 'a URL too')
  t.is(getProxyForUrl('https://www.local.com'), '', 'no_proxy is consulted here')
  t.is(getProxyForUrl('http://origin.example'), '', 'no proxy for this scheme')
  t.is(getProxyForUrl('not a url'), '', 'and nothing that is not a url')
})

test('no_proxy is read from the environment in either case', (t) => {
  t.teardown(withEnv({ NO_PROXY: 'origin.example' }))
  t.ok(bypassed(noProxy(), 'origin.example'))
})
