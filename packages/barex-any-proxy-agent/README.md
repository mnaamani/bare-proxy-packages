...
[`barex-socks-proxy-agent`](../barex-socks-proxy-agent),
[`barex-http-proxy-agent`](../barex-http-proxy-agent) and
[`barex-https-proxy-agent`](../barex-https-proxy-agent), and the shared half in
[`barex-proxy-agent`](../barex-proxy-agent).

```
npm i barex-any-proxy-agent
```
...
Together with [`barex-proxy-from-env`](../barex-proxy-from-env), which is the pairing Node's
...
Re-exported from [`barex-proxy-agent`](../barex-proxy-agent), so a program that dispatches
